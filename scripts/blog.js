...
function hideBlogs(c) {
    for (i = showList.length; i >= 0; i--) {
        if (showList[i] == c) {
            showList.splice(i, 1);
        }
    }
...
function showBlogs(c) {
    showList[showList.length] = c;
    refreshBlogList();
}

$(".show-ai-blog").change(function () {
    $(this).prop('checked') ? showBlogs(".ai-blog") : hideBlogs(".ai-blog");
});
$(".show-cloud-blog").change(function () {
    $(this).prop('checked') ? showBlogs(".cloud-blog") : hideBlogs(".cloud-blog");
});
$(".show-code-blog").change(function () {
    $(this).prop('checked') ? showBlogs(".code-blog") : hideBlogs(".code-blog");
});
$(".show-architecture-blog").change(function () {
    $(this).prop('checked') ? showBlogs(".architecture-blog") : hideBlogs(".architecture-blog");
});
